...
function AllLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Store session data with login type 'all'
        ...
        <div className="relative hidden lg:flex lg:w-5/12 xl:w-1/2 bg-[#137fec] items-center justify-center p-6 overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col items-start max-w-lg">
            ...
            <p className="text-white/80 text-sm leading-relaxed mb-4">
              Access your personalized dashboard, manage your account, and stay connected with your academic journey.
            </p>
          ...
          <div className="w-full max-w-md">
            ...
            {savedCreds.length > 0 && (
              <div className="mb-4 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700">
                <p className="text-xs font-medium text-slate-700 dark:text-slate-300 mb-2">Saved Credentials:</p>
                <div className="space-y-2">
                  {savedCreds.map((cred, index) => (
                    <div
                      key={index}
                      onClick={() => handleUseSavedCreds(cred.email, cred.password)}
                      className="flex items-center justify-between p-2 bg-white dark:bg-slate-700 rounded-lg cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-600 transition-colors border border-slate-200 dark:border-slate-600"
                    >
                      <div className="flex items-center gap-2 flex-1">
                        <span className="material-symbols-outlined text-sm text-[#137fec]">account_circle</span>
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-medium text-slate-900 dark:text-white truncate">{cred.email}</p>
                          <p className="text-xs text-slate-500 dark:text-slate-400">Click to use</p>
                        </div>
                      </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div className="relative group">
                ...
                <div className="flex items-center border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-800/50 focus-within:border-[#137fec] focus-within:ring-1 focus-within:ring-[#137fec] transition-all">
                  <span className="material-symbols-outlined pl-2 text-slate-400 text-base">email</span>
                  <input
                    name="email"
              ...
              <div className="pt-2">
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-[#137fec] hover:bg-[#137fec]/90 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold py-2 rounded-lg shadow-lg shadow-[#137fec]/20 transition-all flex items-center justify-center gap-2 text-sm"
                >
                  {loading ? (
                    <>
